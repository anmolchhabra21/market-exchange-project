...
export class Engine {
    ...
    createOrder(market: string, price_pu: string, quantity: string, side: "buy" | "sell", userId: string) {
        ...
        console.log("update balance", this.balances);

        this.createDbTrades(fills, market, userId);
        console.log("create db trades");
        this.updateDbOrders(order, executedQty, fills, market);
        console.log("update db orders");
        this.publishWsDepthUpdates(fills, price_pu, side, market);
        console.log("publish ws depth updates");
        this.publishWsTrades(fills, userId, market);
        console.log("publish ws trades");
    ...
    checkAndLockBalance(baseAsset: string, quoteAsset: string, side: "buy" | "sell", userId: string, asset: string, price: string, quantity: string) {
        if (side === "buy") {
    ...
    // some users need to have a lot of INR
    setBaseBalances() {
        this.balances.set("1", {
            [BASE_CURRENCY]: {
                available: 10000000,
